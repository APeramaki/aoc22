...
fn main() {
    ...
    loop {        
        ...
        total_score += if overlaps(areas) { 1 } else { 0 };
    }
    println!("{:?}", total_score);
}
...
fn overlaps (areas : [[usize; 2]; 2]) -> bool {
    if areas[0][0] >= areas[1][0] && areas[0][0] <= areas[1][1]{
        return true; // First is (atleast partially) inside second
    }
    if areas[1][0] >= areas[0][0] && areas[1][0] <= areas[0][1] {
        return true; // Second is (atleast partially) inside first
    }
    false
}